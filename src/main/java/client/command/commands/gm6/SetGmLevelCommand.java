...
public class SetGmLevelCommand extends Command {
    {
        setDescription("Set command access level of a player.");
    }
    ...
    public void execute(Client c, String[] params) {
        ...
        if (params.length < 2) {
            player.yellowMessage("Syntax: !setgmlevel <playername> <newlevel 0-6>");
            return;
        }
        ...
        try {
            ...
            player.yellowMessage("Access level must be a number from 0 to 6.");
            return;
        }
        ...
        if (target != null) {
            ...
            target.dropMessage("Your command access level is now " + target.gmLevel()
                    + ". See @commands for available commands.");
            player.dropMessage(target.getName() + " now has command access level " + target.gmLevel() + ".");
        } else {
